...
import { faEdit, faCheck, faTimes, faSearch, faTrash, faSortUp, faSortDown } from '@fortawesome/free-solid-svg-icons';
...
function ListeCarteFidelite() {
    ...
    const fetchCarteFidelite = async () => {
        try {
            const response = await axios.get('http://localhost:3001/cartefidelite');
    ...
    const deleteCarteFidelite = async (carteFideliteId) => {
        try {
            await axios.delete(`http://localhost:3001/cartefidelite/${carteFideliteId}`);
            const updatedCarteFidelite = carteFidelite.filter(carte => carte.id !== carteFideliteId);
            setCarteFidelite(updatedCarteFidelite);
            setFilteredCarteFidelite(updatedCarteFidelite.filter(carte => carte.id !== carteFideliteId));
    ...
    const renderField = (carte, field) => {
        ...
        if (editingField && editingField.carteId === carte.id && editingField.field === field) {
            return (
                <td>
                    <input
                        type="text"
                    ...
                    <FontAwesomeIcon
                        icon={faCheck}
                        className="edit-icon"
                        onClick={() => handleVerifyEdit(carte.id, field, carte[field])}
                    />
                    <FontAwesomeIcon
                        icon={faTimes}
                        className="edit-icon"
    ...
    const handleFieldChange = (e, field) => {
        const updatedCarteFidelite = carteFidelite.map(carte =>
            carte.id === editingField.carteId ? { ...carte, [field]: e.target.value } : carte
        );
        setCarteFidelite(updatedCarteFidelite);
    };
    ...
    const handleVerifyEdit = async (carteId, field, newValue) => {
        ...
        if (confirmed) {
            try {
                await axios.put(`http://localhost:3001/cartefidelite/${carteId}`, { [field]: newValue });
                alert('Carte de fidélité updated successfully');
                setEditingField(null);
                fetchCarteFidelite(); // Refresh the carteFidelite list after update
            } catch (error) {
                if (error.response && error.response.data && error.response.data.error) {
                    alert(`Error updating ${field}: ${error.response.data.error}`);
                } else {
                    alert(`Error updating ${field}`);
                }
                console.error(`Error updating ${field} for carte de fidélité ${carteId}:`, error);
            }
        }
    };
        ...
        <div>
            ...
            <div className="container">
                ...
                <div className="search-container">
                    ...
                    <FontAwesomeIcon 
                        icon={faSearch} 
                        onClick={() => filterAndSortCarteFidelite(searchQuery, sortCriteria, sortOrder)} 
                ...
                <Table striped bordered hover>
                    ...
                    <tbody>
                        {filteredCarteFidelite.length === 0 ? (
                            <tr>
                                <td colSpan="9">No carte de fidélité found</td>
                            </tr>
                        ) : (
                            filteredCarteFidelite.map((carte, index) => (
                                <tr key={carte.id}>
                                    <td>{index + 1}</td>
                                    <td>{carte.id}</td>
                                    {renderField(carte, 'point')}
                                    {renderField(carte, 'reste')}
                                    ...
                                    <td>
                                        <FontAwesomeIcon
                                            ...
                                            style={{ color: "#ed0c0c" }}
                                            onClick={() => deleteCarteFidelite(carte.id)}
                                        />
                                    </td>
                                </tr>
                            ))
