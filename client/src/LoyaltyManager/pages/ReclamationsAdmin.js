...
const ReclamationsAdmin = () => {
	...
	const fetchReclamations = async () => {
		try {
			const response = await axios.get(getBackendURL(`/reclamation`));
	...
	const handleUpdateReclamation = async (idReclamation) => {
		try {
			await axios.put(getBackendURL(`/reclamation/${idReclamation}`), {
				réponse,
			});
			// Update the local state with the updated reclamation
			setReclamations(
				reclamations.map((reclamation) =>
					reclamation.id === idReclamation
						? { ...reclamation, réponse }
						: reclamation
				)
			);
	...
	const startUpdatingReclamation = (idReclamation) => {
		...
		const reclamationToUpdate = reclamations.find(
			(reclamation) => reclamation.id === idReclamation
		);
											...
											{reclamation.statut === "traité" ? (
												...
												<>
													...
													reclamation.id ? (
														<div className="reclamation-bttns">
															<Button
																className="reclamations-sauvegarder-bttn"
																onClick={() =>
														...
														<div className="reclamation-bttns">
															<Button
																className="reclamation-edit-bttn"
																onClick={() =>
																	startUpdatingReclamation(
																		reclamation.id
																	)
																}
															>
													...
													<div className="reclamation-bttns">
														<Button
															className="reclamation-traite-bttn"
															onClick={() =>
																handleMarkAsTraité(
																	reclamation.id
																)
