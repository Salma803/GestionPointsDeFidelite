...
const ListeChequesCadeau = () => {
	...
	const fetchChequeCadeaux = async () => {
		try {
			const response = await axios.get(getBackendURL(`/chequecadeau`));
			setCheques(response.data);
			setFilteredCheques(response.data);
		} catch (error) {
			console.error(
				"Erreur lors de la récupération des chéques cadeaux:",
				error
			);
		}
	};
	...
	const handleUpdate = async (id) => {
		try {
			...
			setCheques(
				cheques.map((item) => (item.id === id ? updatedCheque : item))
			);
			setFilteredCheques(
				filteredCheques.map((item) =>
					item.id === id ? updatedCheque : item
				)
			);
			...
			console.error(
				"Erreur lors de la mise à jour du chéque cadeau:",
				error
			);
		}
	};
	...
	const handleSort = () => {
		const sortedData = [...filteredCheques].sort((a, b) => {
			if (sortOrder === "ASC") {
				return (
					new Date(a.date_expiration) - new Date(b.date_expiration)
				);
			} else {
				return (
					new Date(b.date_expiration) - new Date(a.date_expiration)
				);
			}
		});
	...
	const deleteCheque = async (chequeId) => {
		try {
			await axios.delete(getBackendURL(`/chequecadeau/${chequeId}`));
			setCheques(cheques.filter((cheque) => cheque.id !== chequeId));
			setFilteredCheques(
				filteredCheques.filter((cheque) => cheque.id !== chequeId)
			);
