...
function Panier() {
		...
		const fetchCartItems = async () => {
			try {
				...
				const responsePanier = await axios.get(
					getBackendURL(`/panierenligne/${userId}`)
				);
	...
	const handleAddQuantity = async (id) => {
		try {
			await axios.put(getBackendURL(`/panierenligne/ajouter/${id}`));
			const updatedProducts = products.map((item) =>
				item.id === id ? { ...item, quantité: item.quantité + 1 } : item
			);
	...
	const handleSubtractQuantity = async (id) => {
		try {
			await axios.put(getBackendURL(`/panierenligne/soustraire/${id}`));
			const updatedProducts = products.map((item) =>
				item.id === id ? { ...item, quantité: item.quantité - 1 } : item
			);
			setProducts(updatedProducts);
	...
	const handleDeleteItem = async (id) => {
		try {
			await axios.delete(getBackendURL(`/panierenligne/${id}`));
	...
	const handleClearCart = async () => {
		try {
			...
			await axios.delete(
				getBackendURL(`/panierenligne/client/${userId}`)
			);
			setProducts([]);
			setTotal(0);
		} catch (error) {
	...
	const handlePurchaseCart = async () => {
		try {
			...
			await axios.post(getBackendURL(`/panierenligne/achat/${userId}`));
	...
	const updateLoyaltyCard = async () => {
		try {
			...
			await axios.post(getBackendURL(`/cartefidelite/${userId}`));
		} catch (error) {
						...
						<div className="client-panier-elements">
							<div className="client-cf-screen__content">
										...
										<Col md={4} key={item.id}>
											<Card className="client-panier-element">
												<Card.Body className="client-panier-element-body">
													<Card.Title className="client-panier-element-titre">
														{item.Produit.nom}
													</Card.Title>
													<Card.Text className="client-panier-element-PU">
														Prix unitaire:{" "}
														{
															item.Produit
																.prixApresSolde
													...
													<Card.Text className="client-panier-element-Qte">
														Quantité:{" "}
														{item.quantité}
													</Card.Text>
													<Card.Text className="client-panier-element-Total">
														Total:{" "}
														{item.quantité *
															item.Produit
																.prixApresSolde}{" "}
													...
													<div className="d-flex justify-content-between">
														<PlusCircle
															className="client-panier-add-button"
															size={30}
															onClick={() =>
																handleAddQuantity(
																	item.id
														...
														/>
														<MinusCircle
															className="client-panier-subtract-button"
															size={30}
															onClick={() =>
														...
														/>
														<Trash
															className="client-panier-delete-button"
															size={30}
															onClick={() =>
																handleDeleteItem(
																	item.id
								...
								<Alert variant="info" className="mt-3">
									Total du panier: {total} DH
								</Alert>
							</div>
							<div className="d-flex justify-content-between mt-3">
								<XCircle
									className="client-panier-clear-button"
									size={40}
									onClick={handleClearCart}
								/>
								<ShoppingBag
									className="client-panier-purchase-button"
									size={40}
