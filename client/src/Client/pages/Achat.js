			...
			const achatsWithDetails = await Promise.all(
				achatsData.map(async (achat) => {
					const detailsResponse = await axios.get(
						getBackendURL(`/achat/detail/${achat.id}`)
					);
					const detailsData = Array.isArray(detailsResponse.data)
				...
				<div className="client-achats-container">
					...
					<div className="outils-achats">
						<Form className="client-outils-achat">
							<Form.Group
								controlId="search"
								className="client-achat-recherche"
							>
								<Form.Control
									type="text"
							...
							<Form.Group
								controlId="sortOrder"
								className="client-achat-sort"
							>
								<Form.Label className="tri-label">
									Trier par
								</Form.Label>
								<Form.Control
									as="select"
					...
					{!loading &&
						!error &&
						filteredAndSortedAchats.length === 0 && (
						...
						filteredAndSortedAchats.length > 0 && (
							<div className="client-achats">
								<div className="client-cf-screen__content">
									...
									<Row>
										{filteredAndSortedAchats.map(
											(achat) => (
												<Col key={achat.id}>
													<Card
														className={`client-achat-card ${
															showDetails[
																achat.id
															]
																? "expanded"
													...
													>
														<Card.Body className="achat-body">
															<Card.Title className="client-achat-title">
																Date:{" "}
																{new Date(
																	achat.date_achat
																).toLocaleString()}
															</Card.Title>
															<Card.Text className="client-achat-points">
																Points:{" "}
																{achat.point}
															</Card.Text>
															<Card.Text className="client-achat-total">
																Total:{" "}
																{
																	achat.total_achat
																}{" "}
															...
															<Button
																className="client-achat-details-bttn"
																onClick={() =>
																	toggleDetails(
																		achat.id
																	)
																		...
																		>
																			<thead>
																				<tr>
																					...
																					<th className="client-achat-header-ean1">
																						EAN
																					</th>
																					<th className="client-achat-header-prix">
																						Prix
																					</th>
																					<th className="client-achat-header-qte">
																						Qté
																					</th>
																					<th className="client-achat-header-point">
																						Point
																					</th>
																					<th className="client-achat-header-total">
																						Total
																					</th>
																				</tr>
																			</thead>
																			<tbody>
																				{achat.details.map(
																					(
																						...
																						>
																							<td className="client-achat-td-produit">
																								{
																									detail
																										.produit
																										.nom
																							...
																							<td className="client-achat-td-ean1">
																								{
																									detail
																										.produit
																										.ean1
																							...
																							<td className="client-achat-td-prix">
																								{
																									detail
																										.produit
																										.prix
																							...
																							<td className="client-achat-td-qte">
																								{
																									detail.quantite
																								}
																							</td>
																							<td className="client-achat-td-point">
																								{
																									detail.point
																								}
																							</td>
																							<td className="client-achat-td-total">
																								{
																									detail.total
																								}
																							</td>
