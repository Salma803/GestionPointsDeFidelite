...
import { Alert,Form, Button, Table, Container, Row, Col } from 'react-bootstrap';
...
const Reclamations = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:3001/client/me', {
                    headers: {
                ...
                const reclamationsResponse = await axios.get(`http://localhost:3001/reclamation/${idClient}`);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            await axios.post(`http://localhost:3001/reclamation/${idClient}`, { objet, contenu });
            setObjet('');
            setContenu('');
...
<div className='client-reclamations-body'>
            ...
            <div className="client-cf-screen__background">
                ...
                <Row >
                    <Col>
                        <div className='client-reclamation-header' >Créer une nouvelle réclamation?</div>
                        <Form onSubmit={handleSubmit}>
                            <Form.Group className='client-reclamation-form-objet' controlId="formObjet">
                                <Form.Label>Objet</Form.Label>
                                <Form.Control className='client-objet-input'
                                    type="text"
                                    value={objet}
                            ...
                            <Form.Group className='client-reclamation-form-contenu' controlId="formContenu" >
                                <Form.Label>Contenu</Form.Label>
                                <Form.Control className='client-contenu-input'
                                    type="text"
                                    value={contenu}
                            ...
                            <Button className='send-button-reclamation' type="submit" >
                                Envoyer
                            </Button>
                            </div>
                        </Form>
                    ...
                    <div className='client-table-reclamation'>
                        ...
                        {reclamations.length > 0 ? (
                            <Table  striped bordered hover>
                                <thead>
                                    <tr>
                                        <th className='client-reclamation-table-header'>ID</th>
                                        <th className='client-reclamation-table-header'>Objet</th>
                                        <th className='client-reclamation-table-header'>Contenu</th>
                                        <th className='client-reclamation-table-header'>Statut</th>
                                        <th className='client-reclamation-table-header'>Réponse</th>
                                        <th className='client-reclamation-table-header'>Date de création</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {reclamations.map((reclamation) => (
                                        <tr
                                            key={reclamation.id}
                                            className={reclamation.statut === 'traité' ? 'traité' : 'non-traité'}
                                        >
                                            <td>{reclamation.id}</td>
                                            <td>{reclamation.objet}</td>
                                            <td>{reclamation.contenu}</td>
                                            <td>{reclamation.statut}</td>
                                            <td>{reclamation.réponse === null ? 'Une réponse sera à votre disposition dans peu de temps' : reclamation.réponse}</td>
