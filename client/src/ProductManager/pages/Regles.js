...
function Regles() {
    ...
    useEffect(() => {
        const fetchRegles = async () => {
            try {
                const response = await axios.get('http://localhost:3001/regle');
    ...
    const handleFieldChange = (event, field) => {
        ...
        setRegles(prevRegles =>
            prevRegles.map(regle =>
                regle.id === editingField.regleId ? { ...regle, [field]: value } : regle
            )
        );
    };
    ...
    const handleVerifyEdit = async (regleId, field, newValue) => {
        ...
        if (confirmed) {
            try {
                const regleToUpdate = regles.find(regle => regle.id === regleId);
                const updatedRegle = {
                    ...regleToUpdate,
                    [field]: newValue,
                };
                ...
                await axios.put(
                    `http://localhost:3001/regle/${regleId}`,
                    updatedRegle,
                    {
                        headers: {
    ...
    const renderField = (regle, field) => {
        if (editingField && editingField.regleId === regle.id && editingField.field === field) {
            return (
                <td>
                    <input
                        type="text"
                    ...
                    <FontAwesomeIcon
                        icon={faCheck}
                        className="edit-icon"
                        onClick={() => handleVerifyEdit(regle.id, field, regle[field])}
                    />
                    <FontAwesomeIcon
                        icon={faTimes}
                        className="edit-icon"
