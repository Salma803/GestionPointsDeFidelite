...
function PromotionsProduits() {
	...
	const fetchProduits = async () => {
		try {
			const response = await axios.get(
				getBackendURL(`/produit/produitpromotion`)
			);
		...
		if (confirmed) {
			try {
				...
				if (field === "valeur_promotion") {
					...
					updatedData.date_fin = newValue
						? new Date(newValue).toISOString().split("T")[0]
						: null;
				}

				await axios.put(
					getBackendURL(`/promotion/produit/${produitId}`),
					updatedData,
					{
						headers: {
	...
	const handleFieldChange = (event, field) => {
		...
		setProduits((prevProduits) =>
			prevProduits.map((produit) =>
				produit.id === editingField.produitId
					? { ...produit, [field]: value }
					: produit
			)
		);
	...
	const renderField = (produit, field) => {
		if (
			editingField &&
			editingField.produitId === produit.id &&
			...
			return (
				<td>
					{field === "date_debut" || field === "date_fin" ? (
						<DatePicker
							selected={
								produit[field] ? new Date(produit[field]) : null
							}
					...
					<FontAwesomeIcon
						...
						className="edit-icon"
						onClick={() =>
							handleVerifyEdit(produit.id, field, produit[field])
						}
					/>
					<FontAwesomeIcon
						icon={faTimes}
			...
			return (
				<td>
					{field === "date_debut" || field === "date_fin"
						? produit[field]
							? new Date(produit[field]).toLocaleDateString()
							: ""
						: produit[field]}
					<FontAwesomeIcon
						icon={faEdit}
