...
function Rayons() {
    ...
    const deleteRayon = async (rayonId) => {
        try {
            await axios.delete(`http://localhost:3001/rayon/${rayonId}`);
            setRayons(rayons.filter(rayon => rayon.id !== rayonId));
            setFilteredRayons(filteredRayons.filter(rayon => rayon.id !== rayonId));
    ...
    const handleVerifyEdit = async (rayonId, field, newValue) => {
        ...
        if (confirmed) {
            try {
                await axios.put(
                    `http://localhost:3001/rayon/${rayonId}`,
                    { [field]: newValue },
                    {
                        headers: {
                ...
                if (error.response.status === 401) {
                    ...
                    alert("Une erreur s'est produite lors de la mise à jour du rayon");
                }
            }
        }
    };
