...
function PromotionsRayons() {
    ...
    const fetchRayons = async () => {
        try {
            const response = await axios.get('http://localhost:3001/rayon/promotion');
        ...
        if (confirmed) {
            try {
                ...
                if (field === 'valeur_promotion') {
                    ...
                    updatedData.date_fin = new Date(newValue).toISOString().split('T')[0];
                }
                await axios.put(
                    `http://localhost:3001/promotion/rayon/${rayonId}`,
                    updatedData,
                    {
                        headers: {
                ...
                alert("Une erreur s'est produite lors de la mise à jour de la promotion");
            }
        }
    };
    ...
    const handleFieldChange = (event, field) => {
        ...
        setRayons(prevRayons =>
            prevRayons.map(rayon =>
                rayon.id === editingField.rayonId ? { ...rayon, [field]: value } : rayon
            )
        );
    };
    ...
    const handleDateChange = (date, field) => {
        setRayons(prevRayons =>
            prevRayons.map(rayon =>
                rayon.id === editingField.rayonId ? { ...rayon, [field]: date.toISOString().split('T')[0] } : rayon
            )
        );
    };

    const renderField = (rayon, field) => {
        if (editingField && editingField.rayonId === rayon.id && editingField.field === field) {
            return (
                <td>
                    ...
                    <FontAwesomeIcon
                        icon={faCheck}
                        className="edit-icon"
                        onClick={() => handleVerifyEdit(rayon.id, field, rayon[field])}
                    />
                    <FontAwesomeIcon
                        icon={faTimes}
                        className="edit-icon"
        ...
        <div>
            ...
            <div className='client-cf-screen__content'>
                ...
                <div className="sort-buttons">
                    <button onClick={() => handleSortChange('valeur_promotion')}>
                ...
                <Table className="table" striped bordered hover responsive>
                    ...
                    <tbody>
                        {filteredRayons.length === 0 ? (
                            <tr>
                                <td colSpan="7" className="no-rayons-message">No rayons found</td>
                            </tr>
                        ) : (
                            filteredRayons.map((rayon, index) => (
                                <tr key={rayon.id}>
                                    ...
                                    <td>{new Date(rayon.createdAt).toLocaleDateString()}</td>
                                    <td>{new Date(rayon.updatedAt).toLocaleDateString()}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
