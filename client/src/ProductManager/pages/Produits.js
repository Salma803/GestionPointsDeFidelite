...
function Produits() {
    ...
    useEffect(() => {
        const fetchRayons = async () => {
            try {
                const response = await axios.get('http://localhost:3001/rayon');
    ...
    const fetchProducts = async () => {
        try {
            const response = await axios.get('http://localhost:3001/produit/produits');
    ...
    const handleVerifyEdit = async (productId, field, newValue) => {
        ...
        if (confirmed) {
            try {
                await axios.put(
                    `http://localhost:3001/produit/${productId}`,
                    { [field]: newValue },
                    {
                        headers: {
    ...
    const handleFieldChange = (event, field) => {
        ...
        setProducts(prevProducts =>
            prevProducts.map(product =>
                product.id === editingField.productId ? { ...product, [field]: value } : product
            )
        );
    };
    ...
    const handleRayonChange = (event) => {
        ...
        setProducts(prevProducts =>
            prevProducts.map(product =>
                product.id === editingField.productId ? { ...product, id_rayon: value } : product
            )
        );
    };

    const renderField = (product, field) => {
        if (editingField && editingField.productId === product.id && editingField.field === field) {
            if (field === 'id_rayon') {
                return (
                    <td>
                        <select value={product.id_rayon} onChange={handleRayonChange}>
                            {rayons.map((rayon) => (
                        ...
                        <FontAwesomeIcon
                            icon={faCheck}
                            className="edit-icon"
                            onClick={() => handleVerifyEdit(product.id, field, product.id_rayon)}
                        />
                        <FontAwesomeIcon
                            icon={faTimes}
                            className="edit-icon"
                ...
                return (
                    <td>
                        ...
                        <FontAwesomeIcon
                            icon={faCheck}
                            className="edit-icon"
                            onClick={() => handleVerifyEdit(product.id, field, product[field])}
                        />
                        <FontAwesomeIcon
                            icon={faTimes}
                            className="edit-icon"
    ...
    return (
        <div>
            ...
            <div className='client-cf-screen__content'>
                ...
                <Table className='admin-produits-table' striped bordered hover responsive>
                    <thead>
                        <tr>
                            <th>Nom</th>
                            <th>EAN1</th>
